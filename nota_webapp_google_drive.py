...
def is_bad_person_candidate(value):
    v = re.sub(r"\s+", " ", str(value or "")).strip().lower()
    if not v:
        return True
    bad_phrases = [
        "fino alla revoca", "alla revoca", "data di", "data iscrizione", "data costituzione",
        "data atto", "durata carica", "carica", "nato a", "nata a", "codice fiscale",
        "legale rappresentante", "amministratore unico", "rappresentante legale", "non sono stati rilevati",
        "sede legale", "via ", "viale ", "piazza ", "corso ", "provincia", "comune"
    ]
    return any(v.startswith(p) or p in v for p in bad_phrases)


def looks_like_person_name(value):
    v = re.sub(r"\s+", " ", str(value or "")).strip()
    if not v or is_bad_person_candidate(v):
        return False
    bad = {"fino","alla","revoca","carica","amministratore","unico","legale","rappresentante","nominativo","nome","cognome","sede","via","viale","piazza","corso","socio","protesti","nessun","non","sono","stati","rilevati","attiva","data","durata","iscrizione","costituzione","comune","provincia"}
...
def clean_person_name(value):
    v = str(value or "").strip()
    v = re.sub(r"\s+", " ", v)
    v = re.sub(r"(?i)(cognome\s+e\s+nome|nome\s+e\s+cognome|nominativo|amministratore\s+unico|legale\s+rappresentante|rappresentante\s+legale)\s*[:\-]?", "", v).strip()
    v = re.split(r"(?i)\b(codice\s+fiscale|cod\.?\s*fisc\.?|cf\b|c\.f\.|nato|nata|residente|carica|dal|domicilio|indirizzo|pec|fino\s+alla\s+revoca|fino\s+al\s+|revoca|durata\s+in\s+carica|data\s+di|data\s+iscrizione|data\s+costituzione|data\s+atto)\b", v)[0].strip()
    v = re.sub(r"[^A-Za-zÀ-ÿ'’\.\- ]", " ", v)
    v = re.sub(r"\s+", " ", v).strip()
    if is_bad_person_candidate(v):
        return ""
    parts = [p for p in v.split() if re.match(r"^[A-Za-zÀ-ÿ'’\.-]+$", p)]
    if len(parts) >= 2:
...
def extract_administrator_from_text(text):
    ...
    patterns = [
        r"(?is)(?:cognome\s+e\s+nome|nome\s+e\s+cognome|nominativo)\s*[:\-]?\s*([A-ZÀ-Ý][A-Za-zÀ-ÿ'’\.-]+(?:\s+[A-ZÀ-Ý][A-Za-zÀ-ÿ'’\.-]+){1,3})\s*(?:codice\s+fiscale|cf\b|c\.f\.|nato|nata|residente|data\s+di)",
        r"(?is)legale\s+rappresentante\s*[:\-]?\s*(?:sig\.?|sig\.ra|dott\.?|dr\.?)?\s*([A-ZÀ-Ý][A-Za-zÀ-ÿ'’\.-]+(?:\s+[A-ZÀ-Ý][A-Za-zÀ-ÿ'’\.-]+){1,3})",
        r"(?is)rappresentante\s+legale\s*[:\-]?\s*(?:sig\.?|sig\.ra|dott\.?|dr\.?)?\s*([A-ZÀ-Ý][A-Za-zÀ-ÿ'’\.-]+(?:\s+[A-ZÀ-Ý][A-Za-zÀ-ÿ'’\.-]+){1,3})",
    ...
    # Cerca su singole righe conservando struttura OCR
    lines = [re.sub(r"\s+", " ", x).strip() for x in raw.splitlines() if re.sub(r"\s+", " ", x).strip()]
    key_re = re.compile(r"(?i)amministratore\s+unico|legale\s+rappresentante|rappresentante\s+legale|organi\s+sociali|cognome\s+e\s+nome|nome\s+e\s+cognome|nominativo")
    name_re = re.compile(r"([A-ZÀ-Ý][A-Za-zÀ-ÿ'’\.-]+(?:\s+[A-ZÀ-Ý][A-Za-zÀ-ÿ'’\.-]+){1,3})")
    for i, line in enumerate(lines):
        if key_re.search(line):
            scan = [line] + lines[i+1:i+7]
            for chunk in scan:
                for cand in name_re.findall(chunk):
                    val = clean_person_name(cand)
                    if val and looks_like_person_name(val):
                        return val

    # Fallback testo appiattito attorno alle keyword
    for kw in ["amministratore unico", "legale rappresentante", "rappresentante legale", "organi sociali", "cognome e nome", "nome e cognome"]:
        for m in re.finditer(re.escape(kw), t, flags=re.I):
            chunk = t[m.start(): m.start()+400]
            for cand in name_re.findall(chunk):
                val = clean_person_name(cand)
                if val and looks_like_person_name(val):
                    return val
...
def repair_existing_company_records():
    ...
    for _, row in rows.iterrows():
        ...
        if testo:
            if (not new_amm) or is_bad_person_candidate(new_amm):
